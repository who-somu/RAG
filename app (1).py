...
import faiss
import numpy as np
# Removed unused BitsAndBytesConfig import
...
st.set_page_config(
    page_title="📄 RAG Chatbot with Phi-3",
    page_icon="🤖",
    layout="wide"
)
...
def load_models():
    ...
    # --- FIX: Switched to a more capable, instruction-tuned small model ---
    ...
    # No need to manually set pad_token for this model
    generator = pipeline(
        "text-generation",
        model=model,
        tokenizer=tokenizer
...
# --- Main App Logic ---
st.title("RAG Chatbot with Phi-3")
...
if user_query := st.chat_input("Ask a question about your document..."):
    ...
    with st.spinner("Generating answer..."):
        ...
        # --- FIX: Reverted to using a chat template, which is correct for Phi-3 ---
        chat = [
        ...
        generation_args = {
            "max_new_tokens": 500, # Increased for potentially longer answers
            "do_sample": True,
            "temperature": 0.7,
            "top_k": 50,
            "top_p": 0.95,
        ...
        # Parse the answer by splitting on the assistant tag
        final_answer = raw_text.split("<|assistant|>")[-1].strip()
